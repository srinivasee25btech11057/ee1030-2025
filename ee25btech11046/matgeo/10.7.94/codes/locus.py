...
locus.function.argtypes = [
    ctypes.POINTER(ctypes.c_double), 
    ctypes.c_int,
    ctypes.POINTER(ctypes.c_double)
    ]
locus.function.restype = None

def get_parabola_from_c(x_coords):
    num_points = len(x_coords)
    x_c = x_coords.astype(np.float64)
    y_c = np.empty(num_points, dtype=np.float64)
...
y_locus = get_parabola_from_c(x_locus)
...
plt.text(center_fixed[0] + 0.2, center_fixed[1], 'C (0, 3)')
