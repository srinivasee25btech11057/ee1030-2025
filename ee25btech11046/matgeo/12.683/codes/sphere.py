...
sphere.function.argtypes = [
    ctypes.POINTER(ctypes.c_double),
    ctypes.POINTER(ctypes.c_double)
]
sphere.function.restype = None

def get_normal_from_c(point):
    p_c = np.array(point, dtype=np.float64)
    n_c = np.zeros(3, dtype=np.float64)
    sphere.function(
        p_c.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
        n_c.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
    )
    return n_c

p = (4, 4, 4)
n = get_normal_from_c(p)
...
ax.text(p[0], p[1], p[2], '   P (4, 4, 4)', color='r')
ax.quiver(0, 0, 0, *n, length = 2, color='red', label=r'$\hat{n}$')
